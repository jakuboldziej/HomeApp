import { io } from "socket.io-client";
import { AppState } from 'react-native';
import * as SecureStore from 'expo-secure-store';
import { getDartsGame, cfAccessHeaders } from "./fetch";

const readSessionToken = async () => {
  try {
    const stored = await SecureStore.getItemAsync("user");
    return stored ? JSON.parse(stored).token : null;
  } catch {
    return null;
  }
};

export const socketUrl = process.env.NODE_ENV === "development" ? process.env.EXPO_PUBLIC_BACKEND_URL_LOCAL : process.env.EXPO_PUBLIC_BACKEND_URL

export const socket = io(socketUrl, {
  autoConnect: false,
  auth: (cb) => { readSessionToken().then((token) => cb({ token })); },
  transports: ['polling', 'websocket'],
  reconnection: true,
  reconnectionDelay: 1000,
  reconnectionDelayMax: 5000,
  reconnectionAttempts: Infinity,
  timeout: 20000,
  forceNew: false,
  upgrade: true,
  extraHeaders: {
    'ngrok-skip-browser-warning': 'true',
    ...cfAccessHeaders
  }
});

let isConnected = false;
let reconnectAttempts = 0;
const maxReconnectAttempts = Infinity;
let appStateSubscription = null;

const activeRooms = new Set();

export const trackRoom = (roomId) => {
  if (roomId) {
    activeRooms.add(roomId);
  }
};

export const untrackRoom = (roomId) => {
  if (roomId) {
    activeRooms.delete(roomId);
  }
};

export const getActiveRooms = () => Array.from(activeRooms);

const handleAppStateChange = (nextAppState) => {
  if (nextAppState === 'active') {
    if (!socket.connected) {
      console.info('App became active, reconnecting socket...');
      socket.connect();
    }
  } else if (nextAppState === 'background') {
    console.info('App went to background');
  }
};

// Start listening to app state changes
if (!appStateSubscription) {
  appStateSubscription = AppState.addEventListener('change', handleAppStateChange);
}

socket.on('connect', async () => {
  isConnected = true;
  const wasReconnecting = reconnectAttempts > 0;
  reconnectAttempts = 0;
  console.info('Socket connected:', socket.id);

  if (activeRooms.size > 0) {
    if (activeRooms.size > 0) {
      for (const roomId of activeRooms) {
        const freshGame = await getDartsGame(roomId).catch(() => null);

        if (freshGame && freshGame.active !== false) {
          console.info("Rejoining active game:", roomId);
          socket.emit("joinLiveGamePreview", JSON.stringify({ gameCode: roomId }));
          break;
        } else {
          console.info("Removing inactive room:", roomId);
          activeRooms.delete(roomId);
        }
      }
    }

  }
});

socket.on('disconnect', (reason) => {
  isConnected = false;
  console.info('Socket disconnected:', reason);

  if (reason === 'io server disconnect' || reason === 'transport close' || reason === 'transport error') {
    setTimeout(() => {
      if (!socket.connected) {
        socket.connect();
      }
    }, 1000);
  }
});

socket.on('connect_error', (error) => {
  reconnectAttempts++;

  if (reconnectAttempts >= 3 && reconnectAttempts % 3 === 0) {
    console.error(`Connection error (attempt ${reconnectAttempts}):`, error.message);
  }
});

socket.on('reconnect', (attemptNumber) => {
  reconnectAttempts = 0;
  console.info(`Socket reconnected after ${attemptNumber} attempts`);
});

socket.on('reconnect_error', (error) => {
  if (reconnectAttempts >= 3) {
    console.error('Reconnection error:', error.message);
  }
});

socket.on('reconnect_failed', () => {
  console.error('Failed to reconnect after all attempts');
});

socket.on('error', (error) => {
  console.error('Socket error:', error);
});

// Heartbeat to keep connection alive
let heartbeatInterval;

const startHeartbeat = () => {
  if (heartbeatInterval) clearInterval(heartbeatInterval);

  heartbeatInterval = setInterval(() => {
    if (socket.connected) {
      socket.emit('heartbeat');
    } else {
      console.info('Heartbeat: Socket disconnected, attempting reconnect...');
      socket.connect();
    }
  }, 25000); // 25 seconds
};

const stopHeartbeat = () => {
  if (heartbeatInterval) {
    clearInterval(heartbeatInterval);
    heartbeatInterval = null;
  }
};

socket.on('connect', startHeartbeat);
socket.on('disconnect', stopHeartbeat);

socket.on('heartbeat-ack', () => {
  // Silent acknowledgment
});

export const isSocketConnected = () => socket.connected && isConnected;

export const ensureSocketConnection = () => {
  if (!socket.connected) {
    console.info('Ensuring socket connection...');
    socket.connect();
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error('Connection timeout'));
      }, 10000);

      socket.once('connect', () => {
        clearTimeout(timeout);
        console.info('Socket connection established');
        resolve();
      });

      socket.once('connect_error', (error) => {
        clearTimeout(timeout);
        console.error('Socket connection error:', error);
        reject(error);
      });
    });
  }
  return Promise.resolve();
};

export const cleanupSocket = () => {
  stopHeartbeat();
  if (appStateSubscription) {
    appStateSubscription.remove();
    appStateSubscription = null;
  }
};